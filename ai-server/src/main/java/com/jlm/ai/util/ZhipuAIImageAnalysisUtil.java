...
import com.jlm.ai.dto.ExamPaperAnalysisDto;
...
public class ZhipuAIImageAnalysisUtil extends AIUtil {
     ...
     * @throws IOException 文件读取或API调用异常
     */
    public List<QuestionAnalysisDto> reviewExamQuestions(String imagePath) throws IOException {
        ...
        List<QuestionAnalysisDto> questionAnalysisList = parseAIResultToQuestionAnalysisList(aiResult);
     ...
     * @throws IOException 文件读取或API调用异常
     */
    public List<HomeworkPublishQuestionDto> reviewHomreWorkQuestions(String imagePath) throws IOException {
        // 构建详细的批阅提示词，要求模型返回结构化信息
        String prompt = "作为一个图片试题分析助手,请详细分析并批阅图片中的所有试题，按题号顺序返回以下信息：\n"
                + "1. 题号： 包含试题大题号、小题号,格式如大题号：小题号：\n"
                + "2. 题型： 试题题型类别\n"
        ...
        //System.out.println("AI分析结果:" + aiResult);
        // 解析AI结果为QuestionAnalysis列表
        List<HomeworkPublishQuestionDto> questionAnalysisList = parseAIResultToHomreWorkQuestionList(aiResult);
        return questionAnalysisList;
    }
     ...
     * @throws IOException 文件读取或API调用异常
     */
    public List<HomeworkPublishQuestionDto> reviewHomreWorkQuestions(List<String> imagePaths) throws IOException {
        // 构建详细的批阅提示词，要求模型返回结构化信息
        String prompt = "作为一个图片试题分析助手,请详细分析并批阅图片中的所有试题，按题号顺序返回以下信息：\n"
                + "1. 题号： 包含试题大题号、小题号,格式如大题号：小题号：\n"
                + "2. 题型： 试题题型类别\n"
    ...
    public List<HomeworkPublishQuestionDto> parseAIResultToHomreWorkQuestionList(String aiResultText) {
        List<HomeworkPublishQuestionDto> questionAnalysisList = new ArrayList<>();
        ...
        try {
            ...
            // 当前正在处理的题目
            HomeworkPublishQuestionDto currentQuestion = null;
            ...
            for (String line : lines) {
                ...
                if (line.contains("大题号") || line.contains("题号")) {
                    ...
                    // 创建新的题目分析对象
                    currentQuestion = new HomeworkPublishQuestionDto();
    ...
    public List<HomeworkPublishQuestionDto> parseAIResultToHomreWorkQuestionStucList(ZhiPuAIAgent.TopicReport report) {
        List<HomeworkPublishQuestionDto> questionAnalysisList = new ArrayList<>();


        List<ZhiPuAIAgent.SubQuestions> answers = report.answers();

        try {
            // 当前正在处理的题目
            HomeworkPublishQuestionDto currentQuestion = new HomeworkPublishQuestionDto();

            for (ZhiPuAIAgent.SubQuestions subQue : answers) {
                String majorQuestionId = subQue.major_question_id();  //大题号
                String questionId = subQue.question_id();
                List<String> answer = subQue.answer_text(); //答案
                String questionType = subQue.question_type();
                String content = subQue.question_content();
                List<String> knowledgePoints = subQue.knowledge_points();
                ...
                currentQuestion.setReferenceAnswer(answer.toArray().toString()); //答案
                currentQuestion.setContent(content); //问题内容
                currentQuestion.setKnowledgePoints(knowledgePoints.toArray().toString()); //参考知识点
                questionAnalysisList.add(currentQuestion);
            }
        } catch (Exception e) {
            // 如果解析失败，记录错误并返回空列表
     ...
     * @throws IOException 文件读取或API调用异常
     */
    public List<QuestionAnalysisDto> batchReviewExamQuestions(List<String> imagePaths) throws IOException, ExecutionException, InterruptedException {
        if (imagePaths == null || imagePaths.isEmpty()) {
            return null;
        }
        ...
        int threadCount = Math.min(5, imagePaths.size()); // 限制最大线程数
        ExecutorService executorService = Executors.newFixedThreadPool(threadCount);
        List<QuestionAnalysisDto> results = new ArrayList<>();
        Map<String, List<QuestionAnalysisDto>> futureResults = new HashMap<>();
        ...
        try {
            ...
            // 收集所有结果
            for (Map.Entry<String, List<QuestionAnalysisDto>> entry : futureResults.entrySet()) {
                String imagePath = entry.getKey();
                try {
                    List<QuestionAnalysisDto> result = entry.getValue(); // 获取分析结果，会阻塞直到完成
     ...
     */
    public List<QuestionAnalysisDto> parseAIResultToQuestionAnalysisList(String aiResultText) {
        List<QuestionAnalysisDto> questionAnalysisList = new ArrayList<>();
        ...
        try {
            ...
            // 当前正在处理的题目
            QuestionAnalysisDto currentQuestion = null;
            ...
            for (String line : lines) {
                ...
                if (line.contains("大题号") || line.contains("题号")) {
                    ...
                    // 创建新的题目分析对象
                    currentQuestion = new QuestionAnalysisDto();
            ...
            if (firstChoice.has("message") && firstChoice.get("message").has("content")) {
                ...
                if (rootNode.has("usage")) {
                    ...
                    if (usageNode.has("prompt_tokens"))
                        usageInfo.put("prompt_tokens", usageNode.get("prompt_tokens").asInt());
                    if (usageNode.has("completion_tokens"))
                        usageInfo.put("completion_tokens", usageNode.get("completion_tokens").asInt());
                    if (usageNode.has("total_tokens"))
                        usageInfo.put("total_tokens", usageNode.get("total_tokens").asInt());
     ...
     * @return 结构化的试卷分析对象
     */
    public ExamPaperAnalysisDto parseTextToExamPaperAnalysis(String aiResultText) {
        ExamPaperAnalysisDto analysis = new ExamPaperAnalysisDto();
    ...
    public static void main(String[] args) {
        ...
        List<QuestionAnalysisDto> analyses = null;
        try {
